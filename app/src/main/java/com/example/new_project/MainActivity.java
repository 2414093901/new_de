        ...
        switch (view.getId()){
            case R.id.main_dl:
                ...
                if (sjh.length()<6 || mima.length()<6){
                    Toast.makeText(MainActivity.this,"账号或密码少于6位",Toast.LENGTH_SHORT).show();
                    mainMm.setText("");
                   // mainSjh.setText("");
                }else {
                    ...
                    HttpClick.getInstance().get(Http_url.URL_log, requestParams, new HttpClick.IHttpRequestListener() {
                        @Override
                        public void IOnSuccess(String resouces) {
                            Login_base login_base = JSON.parseObject(resouces, Login_base.class);
                            if (login_base.getCode()==200) {
                               String photoPath = login_base.getData().getUimg();
                                ...
                                Toast.makeText(MainActivity.this,"登陆失败"+resouces,Toast.LENGTH_SHORT).show();
                            }
                        }

                        @Override
                        public void IOnFailure(String resouces) {
                            Log.e("111","登陆失败"+resouces);
                        }
                    });
                }
                break;
